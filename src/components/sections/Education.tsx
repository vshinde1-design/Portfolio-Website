...
const HonorsSection: React.FC = () => {
  ...
  const [bgsBGLightboxOpen, setBgsBGLightboxOpen] = React.useState(false)
      ...
      <div className="relative z-10 mt-8 flex flex-col gap-6">
        ...
        >
          <Card className="w-full flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6 p-4 md:p-6">
            ...
            <div className="flex-shrink-0 self-center mt-3 md:mt-0 w-56 md:w-64 h-44 md:h-52 rounded-xl border border-blue-400/40 p-[3px]">
              <button
                onClick={() => setBgsBGLightboxOpen(true)}
                className="block w-full h-full rounded-[10px] overflow-hidden focus:outline-none group"
                aria-label="View Beta Gamma Sigma certificate"
              >
                <img
                  src="/certs/BGS Cert.png"
                  alt="Beta Gamma Sigma Membership Certificate"
                  className="w-full h-full object-contain transition-transform duration-300 group-hover:scale-105"
                  onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none' }}
                />
              </button>
            ...
            <AnimatePresence>
              {bgsBGLightboxOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  transition={{ duration: 0.2 }}
                  onClick={() => setBgsBGLightboxOpen(false)}
                >
                  <button
                    onClick={() => setBgsBGLightboxOpen(false)}
                    className="absolute top-4 right-4 flex items-center justify-center w-10 h-10 rounded-full bg-white/15 border border-white/25 text-white hover:bg-white/25 transition-colors text-base font-bold z-50"
                    aria-label="Close"
                  >
                    ✕
                  </button>
